...
wls  = est_wide("TEPCO", "lab_data.estimated_demand_fy2024_wls", "wls_%")
...
j.to_csv(f"{HERE}/wls_g1_tepco.csv", index=False, encoding="utf-8-sig")

def coverage(U, tw):
    sd = np.load(f"{HERE}/wls_sd_{U}.npy")
    ks = pd.read_csv(f"{HERE}/wls_keys_{U}.csv", header=None)[0].astype(str).tolist()
    sdm = dict(zip(ks, sd))
    ew = est_wide(U, "lab_data.estimated_demand_fy2024_wls", "wls_%")
    hit = tot = 0; per = []
    for k in tw.columns:
...
d2.to_csv(f"{HERE}/wls_g2_tepco.csv", index=False, encoding="utf-8-sig")
...
if tk is None: print("  真値なし"); c3, n3 = np.nan, 0
    ...
    d3.to_csv(f"{HERE}/wls_g3_kansai.csv", index=False, encoding="utf-8-sig")
